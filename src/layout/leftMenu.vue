  ...
  <el-row class="menu_page">
    <el-col>
      <el-menu
        ...
        <template v-for="item in $store.state.menu.items" v-if='item.hidden !== true'>
          <el-submenu v-bind:key="item.id" v-if="item.children && !item.noDropdown && item.children.length > 0" :index="item.path">
            <template slot="title">
          ...
          <router-link :to="item.path" v-bind:key="item.id">
